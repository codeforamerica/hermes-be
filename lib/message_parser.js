...
module.exports = function(message, options) {
  ...
  var AFFIRMATION_REGEXP = /^\s?(y|ye|yes|yep|yeah|yah|yeppers|ya|yup|si)\s?$/i
  var NEGATION_REGEXP = /^\s?(n|no|nah|nope|never|nada|nopes|nein)\s?$/i
  ...
  var parseCaseNumber = function(regexpMatchResult) {
    ...
    if (regexpMatchResult[4]) {
      ...
      canonical = sprintf('%02d-%s-%06d-%3d', year, type, serial, defendant)
    } else {
      canonical = sprintf('%02d-%s-%06d', year, type, serial)
    }
