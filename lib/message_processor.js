...
module.exports = function(userNumber, hermesNumber, text, id, receivedAt, options) {
  ...
  var caseDetailsFetcher = options.caseDetailsFetcher || caseDetailsFetcher
  ...
  var findSubscriptionsForContact = function(contact, cb) {
    ...
    models.case_subscription.findAll({
      where: {
        state: [ 'SUBSCRIBED', 'UNCONFIRMED', 'UNCONFIRMED_DELAYED' ],
  ...
  var fetchCaseDetails = function(kase, contact, cb) {
    var fetcher = caseDetailsFetcher(kase.number)
        ...
        .success(function(kase) {
          ...
          models.case_subscription.findOrCreate({
            ...
            .success(function(caseSubscription) {
              ...
              updateInboundConversationWithCase(conversation, kase, util.handleError(cb, function() {

                // Fetch case details
                fetchCaseDetails(kase, contact, function(err, details) {
                  if (err) {
                    // CN could not be reached
                    // --> UNCONFIRMED DELAYED
                    caseSubscription.state = 'UNCONFIRMED_DELAYED'
                    caseSubscription.save()
                      .success(function() { console.log('here'); RESP_OK_CONFIRMATION_DELAYED_ORIGIN_UNREACHABLE(util.handleError(cb, function(outboundMessage) {
                        recordOutboundConversation(contact, null, outboundMessage, cb)
                      })) })
                      .error(function(err) { returnError('Could not set subscription state to UNCONFIRMED_DELAYED', err, cb) })
                  } else if (details.title === null) {
                    console.error('case # ' + kase.number + ' not found in CN.')
                    // Case not found
                    kase.compareNumberWithMax(function(err, compareResult) {
                      if (err) {
                        ...
                        caseSubscription.state = 'UNCONFIRMED_DELAYED'
                        caseSubscription.save()
                          .success(function() { RESP_OK_CONFIRMATION_DELAYED_UNKNOWN_CASE(kase.number, cb) })
                          .error(function(err) { returnError('Could not set subscription state to UNCONFIRMED_DELAYED', err, cb) })
                      } else {
                        // case number is greater than max                        
                        caseSubscription.state = 'UNCONFIRMED_DELAYED'
                        caseSubscription.save()
                          .success(function() { RESP_OK_CONFIRMATION_DELAYED_UNKNOWN_CASE_YET(kase.number, cb) })
                          .error(function(err) { returnError('Could not set subscription state to UNCONFIRMED_DELAYED', err, cb) })
                      }
                    ...
                    caseSubscription.state = 'UNCONFIRMED_DELAYED'
                    caseSubscription.save()
                      .success(function() { RESP_OK_CONFIRMATION_DELAYED_NO_COURT_DATE(cb) })
                      .error(function(err) { returnError('Could not set subscription state to UNCONFIRMED_DELAYED', err, cb) })
                  } else {
  ...
  var respondWithUnknownMessageError = function(contact, cb) {
  ...
  var recordOutboundConversation = function(contact, kase, text, cb) {  
    ...
    message.save()
      .success(function(m) { cb(null, text) })
    ...
    process: function(cb) {
      ...
      async.parallel([
        ...
        findSubscriptionsForContact(contact, util.handleError(cb, function(subscriptions) {
          ...
          recordInboundConversation(contact, currentCase, util.handleError(cb, function(conversation) {
            ...
            // Catch-all: Respond with generic error message
              ...
              console.log(99)
              respondWithUnknownMessageError(contact, cb)
            }
