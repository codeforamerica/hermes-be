...
module.exports = function(from, to, text, id, receivedAt) {
  ...
  var RESP_OK_SUBSCRIBED_TO_CASE = function(caseNumber, cb) {
    renderer.render("resp-ok-subscribed-to-case", { caseNumber: caseNumber }, cb)
  } // END function - RESP_OK_SUBSCRIBED_TO_CASE

  var RESP_ERR_UNKNOWN_MESSAGE = function(cb) {
    renderer.render("resp-err-unknown-message", {}, cb)
            ...
            caseContact.save()
              .success(function(cc) {
                updateInboundConversationWithCase(conversation, kase, handleError(cb, function() {
                  RESP_OK_SUBSCRIBED_TO_CASE(caseNumber, cb) 
                }))
              })
              .error(function(err) {
                updateInboundConversationWithCase(conversation, kase, handleError(cb, function() {
