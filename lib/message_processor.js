...
module.exports = function(from, to, text, id, receivedAt) {
    ...
    models.case_contact.find({
      ...
      .success(function(cc) {
        ...
        if (cc) {
          cc.destroy()
            .success(function() { 
              console.log('Previous unconfirmed subscriptions deleted.')
  ...
  var respondWithCaseConfirmation = function(contact, kase, cb) {
    ...
    models.case_contact.find({
      ...
      .success(function(cc) {
        cc.subscriptionState = 'SUSBCRIBED'
        cc.save()
          .success(function() {
            fetchCaseDetails(caseNumber, function(err, details) {
              if (err) {
                ...
                RESP_OK_CASE_SUBSCRIPTION_CONFIRMED(kase.number, cb)
              }
            })
          })
          .error(function(err) { returnError('Could not delete subscription', err, cb) })
