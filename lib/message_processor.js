...
module.exports = function(userNumber, hermesNumber, text, id, receivedAt, options) {
  ...
  var RESP_OK_ALREADY_GETTING_REMINDERS = function(kase, cb) {

    var replacements = {}
    var defendantName = kase.parseDefendantName()
    if (defendantName) {
      replacements.defendantName = defendantName.firstName + ' ' + defendantName.lastName
      replacements.defendantNameExists = function() { return true }
    } else {
      replacements.defendantNameExists = function() { return false }
    }
    replacements.caseTitle = kase.title
    replacements.caseNumber = kase.number

    renderer.render('already-getting-reminders', replacements, cb)

  } // END function - RESP_OK_ALREADY_GETTING_REMINDERS

  var RESP_OK_HOW_TO_RESUBSCRIBE = function(cb) {
    renderer.render('how-to-resubscribe', {}, cb)
  } // END function - RESP_OK_HOW_TO_RESUBSCRIBE
  ...
  var respondWithConfirmedSubscription = function(contact, kase, cb) {
    ...
    models.case_subscription.find({
      ...
      .success(function(cc) {
        cc.state = 'SUBSCRIBED'
        cc.subscription_datetime = new Date()
        cc.save()
          .success(function() {
            fetchCaseDetails(kase, contact, function(err, details) {
              if (err) {
  ...
  var respondWithResubscriptionInstructions = function(contact, kase, cb) {

    var outboundMessages
    async.parallel([
      function(callback) { RESP_OK_ALREADY_GETTING_REMINDERS(kase, callback) },
      RESP_OK_HOW_TO_RESUBSCRIBE
    ], function(err, results) {

      outboundMessages = results
      async.each(
        outboundMessages,
        function(message, callback) { recordOutboundConversation(contact, kase, message, callback) },
        util.handleError(cb, function() {
          cb(null, outboundMessages)
        })
      ) // END - async.each

    })

  } // END function - respondWithResubscriptionInstructions
      ...
      async.parallel([
        ...
        findSubscriptionsForContact(contact, util.handleError(cb, function(subscriptions) {
          ...
          recordInboundConversation(contact, currentCase, util.handleError(cb, function(conversation, event) {
            ...
            // If contact has no subscription && message is a case #: 
            // find or create case + subscription (in UNCONFIRMED state)
            ...
            // If contact has a subscription && message is
            ...
            // If contact has a subscription in UNCONFIRMED state &&
            // message is AFFIRMATION: subscribe to case
            ...
            // If contact has a subscription it UNCONFIRMED state &&
            // message is NEGATION: unsubsribe UNCONFIRMED case
            else if (currentSubscription &&
                     (currentSubscription.state == 'UNCONFIRMED') &&
            ...
            // If contact has a subscription in UNCONFIRMED or 
            // UNCONFIRMED_DELAYED state && message is a case #: 
            // unsubscribe UNCONFIRMED case and subscribe to new case
            else if (currentSubscription &&
            ...
            // If contact has a subscription in SUBSCRIBED state && 
            // message is the same case #: re-send confirmed message
            else if (currentSubscription &&
                     (currentSubscription.state == 'SUBSCRIBED') &&
                     (parsedMessage.type == parser.MESSAGE_TYPE_CASE_NUMBER) &&
                     (parsedMessage.normalized == currentSubscription.case.number)) {
              respondWithConfirmedSubscription(contact, currentCase, cb)
            }

            // If contact has a subscription in SUBSCRIBED state && 
            // message is a NEW case #: send TWO responses - 1) about
            // already getting reminders and 2) about how to resubscribe
            else if (currentSubscription &&
                     (currentSubscription.state == 'SUBSCRIBED') &&
                     (parsedMessage.type == parser.MESSAGE_TYPE_CASE_NUMBER)) {
              respondWithResubscriptionInstructions(contact, currentCase, cb)
            }
